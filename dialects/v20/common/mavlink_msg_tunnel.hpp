 ...
 * Message for transporting "arbitrary" variable-length data from one component to another
                (broadcast is not forbidden, but discouraged). The encoding of the data is usually extension specific,
                i.e. determined by the source, and is usually not documented as part of the MAVLink specification.
...
struct TUNNEL : mavlink::Message {
    ...
    std::array<uint8_t, 128> payload; /*<  Variable length payload. The payload length is defined by
