 ...
 * Data for filling the OpenDroneID Authentication message. The Authentication Message defines a
                field that can provide a means of authenticity for the identity of the UAS (Unmanned Aircraft System).
                The Authentication message can have two different formats. For data page 0, the fields PageCount, Length
                and TimeStamp are present and AuthData is only 17 bytes. For data page 1 through 15, PageCount, Length
...
struct OPEN_DRONE_ID_AUTHENTICATION : mavlink::Message {
    ...
    uint32_t timestamp; /*< [s] This field is only present for page 0. 32 bit Unix
             ...
             */
    std::array<uint8_t, 23> authentication_data; /*<  Opaque authentication data. For page 0, the size is
