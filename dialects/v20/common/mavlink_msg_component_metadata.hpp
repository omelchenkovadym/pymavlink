 ...
 * 
                ...
                The general metadata file can be read to get the locations of other metadata files (COMP_METADATA_TYPE)
                and translations, which may be hosted either on the vehicle or the internet.
...
struct COMPONENT_METADATA : mavlink::Message {
    ...
    std::array<char, 100> uri; /*<  MAVLink FTP URI for the general metadata file
