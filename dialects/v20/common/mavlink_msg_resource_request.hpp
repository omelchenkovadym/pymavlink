...
struct RESOURCE_REQUEST : mavlink::Message {
    ...
    uint8_t uri_type; /*<  The type of requested URI. 0 = a file via URL. 1 = a UAVCAN binary
             */
    std::array<uint8_t, 120> uri; /*<  The requested unique resource identifier (URI). It is not necessarily
    ...
    uint8_t transfer_type; /*<  The way the autopilot wants to receive the URI. 0 = MAVLink FTP.
                1 = binary stream.
             */
    std::array<uint8_t, 120> storage; /*<  The storage path the autopilot wants the URI to be stored in. Will
